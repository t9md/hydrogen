...
export default class Hydrogen {
  ...
  async restartKernelAndReEvaluateBubbles() {
    ...
    if (kernelManager.hasKernelForEditor(editor)) {
      const restarted = await kernelManager.restart(editor)
      // Early return if it couldn't restart, e.g. Another restarting was in progress.
      if (!restarted) return
    }
    const breakpoints = bubbleManager.getStartPositions(editor)
  ...
  runAllAbove() {
    ...
    const row = codeManager.findNormalRowUpward(editor, {from: cursor.getBufferRow()})
  ...
  runCell({moveDown = false} = {}) {
    ...
    const text = codeManager.normalizeString(
      editor.getTextInBufferRange([[start.row, 0], [row, Infinity]])
    )
    if (!text) return
  ...
  clearBubble() {
    bubbleManager.clear(this.editor)
  }
  ...
  switchKernel(kernelSpec) {
    ...
    // kernelManager.startKernel(editor, grammar, kernelSpec)
  }

  interruptKernel() {
    return kernelManager.interrupt(this.editor)
  }

  restartKernel() {
    return kernelManager.restart(this.editor)
  }

  shutdownKernel() {
    bubbleManager.clear(this.editor)
    return kernelManager.shutdown(this.editor)
  }

  renameWSKernel() {
    return kernelManager.renameWSKernel(this.editor)
  }

  disconnectKernel() {
    bubbleManager.clear(this.editor)
    return kernelManager.disconnect(this.editor)
  }
}
