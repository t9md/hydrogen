...
class Hydrogen {
  ...
  async executeText(editor, text, bubbleRow) {
    ...
    resultView.setMarker(decoration.marker)

    const globalOutputStore = HydrogenPane.findOutputArea()
    const activeOutputStore = globalOutputStore ? globalOutputStore : outputStore
    ...
    const onShellMessage = message => {
      activeOutputStore.appendOutput(message)
  ...
  toggleBubble() {
    ...
    for (let row = startRow; row <= endRow; row++) {
      const cleared = this.bubbleManager.clearMarkersAtRow(row)
      if (cleared) continue

      const outputStore = createOutputStore(editor, row)
      const {element} = new ResultView(outputStore, null, false)
      this.bubbleManager.createBubble({row: row, item: element})
    }
  }
