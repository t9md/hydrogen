...
export default class Hydrogen {
  constructor(editor) {
    this.editor = editor
    this.kernelManager = new KernelManager(editor)
    this.bubbleManager = new BubbleManager(editor)
  }
  ...
  async restartKernelAndReEvaluateBubbles() {
    ...
    const breakpoints = this.bubbleManager.getStartPositions()
  ...
  toggleBubble() {
    ...
    for (let row = startRow; row <= endRow; row++) {
      ...
      for (const marker of this.bubbleManager.findMarkers({startBufferRow: row})) {
        marker.destroy()
        destroyed = true
      }
