...
export default class SignalListView {
  ...
  _mapCommands(commands, kernelSpec) {
  ...
  async toggle() {
    ...
    if (kernel instanceof WSKernel) {
      ...
      const otherKernels = kernelManager.getAllKernelSpecsForGrammar(grammar, kernelSpecs =>
        _.without(kernelSpecs, kernel.kernelSpec)
      )
