...
class ResultViewComponent extends React.Component {
  ...
  addCopyTooltip = (element, comp) => {
    ...
    comp.add(
      atom.tooltips.add(element, {
        title: this.props.outputStore.executionCount
          ? `Copy to clipboard (Out[${this.props.outputStore.executionCount}])`
          : "Copy to clipboard",
      })
    )
  }
  ...
  render() {
    const {outputs, status, isPlain, position} = this.props.outputStore
    ...
    if (outputs.length === 0 || this.props.showResult === false) {
      ...
      return <Status status={statusComputed} style={inlineStyle} />
    }
    ...
    return (
      <div
        ...
        style={isPlain ? inlineStyle : {maxWidth: `${position.editorWidth}ch`, margin: "0px"}}
      >
        <Display
          ref={ref => {
        ...
        {isPlain
          ...
          : <div className="toolbar">
              ...
              />
              <div className="icon icon-file-symlink-file" onClick={this.openInEditor} />
              <div
  ...
  scrollToBottom() {
    if (
      !this.el ||
      this.expanded === true ||
      this.props.outputStore.isPlain === true ||
