...
import {Emitter, CompositeDisposable, Disposable, Range, Point, TextEditor} from "atom"
...
const HydrogenPackage = {
  ...
  getKernel(editor = getEditor()) {
    return kernelManager.getKernelForEditor(editor)
  },
  ...
  async restartKernelAndReEvaluateBubbles() {
    ...
    if (kernelManager.hasKernelForEditor(editor)) {
      const restarted = await this.restartKernel()
      if (!restarted) return
    }
  ...
  async executeText(editor, text, bubbleRow) {
    await kernelManager.startKernelIfNeeded(editor)
    const kernel = this.getKernel(editor)
...
export default HydrogenPackage
