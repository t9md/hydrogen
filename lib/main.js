...
const Hydrogen = {
  ...
  deactivate() {
    ...
    for (const markerLayer of this.markerLayerByEditor.values()) {
      markerLayer.destroy()
    }
  ...
  async restartKernelAndReEvaluateBubbles() {
    ...
    const markerLayer = this.markerLayerByEditor.get(editor)
    if (markerLayer) {
      breakpoints = markerLayer.getMarkers().map(marker => marker.getStartBufferPosition())
    }
  ...
  toggleBubble() {
    ...
    for (let row = startRow; row <= endRow; row++) {
      ...
      if (markerLayer) {
        markerLayer.findMarkers({startBufferRow: row}).forEach(marker => {
          marker.destroy()
          destroyed = true
        })
      }
