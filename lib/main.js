...
const Hydrogen = {
  ...
  async runAll(breakpoints) {
    ...
    await this.startKernelIfNeeded(editor)
  ...
  async startKernelIfNeeded(editor) {
    ...
    if (!kernel) {
      const grammar = getGrammar(editor)
      const kernelSpec = kernelManager.getKernelSpecForGrammar(grammar)
      await kernelManager.startKernel(editor, grammar, kernelSpec)
      started = true
    }
    return started
  },
