...
import {getGrammarWithNormalizeEmbedded} from "./utils"
...
class KernelManager {
  ...
  async start() {
    ...
    const grammar = getGrammarWithNormalizeEmbedded(this.editor)
    const kernelSpec = this.getKernelSpecForGrammar(grammar)

    const kernel = new ZMQKernel(kernelSpec, grammar)
    await kernel.start({cwd: getCWD(this.editor)})
