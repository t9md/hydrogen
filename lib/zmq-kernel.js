...
export default class ZMQKernel extends Kernel {
  ...
  monitorConnect(done) {
    try {
      ...
      for (const socketName of socketNames) {
        connectPendingCount++
        const socket = this.socket[socketName]
  ...
  destroy() {
    ...
    for (const socket of Object.values(this.socket)) {
      socket.close()
    }
    super.destroy()
  }
  ...
  sendMessage(socketName, {msgType, msgId, content, handler}) {
    ...
    this.socket[socketName].send(new Message(message))
  }
