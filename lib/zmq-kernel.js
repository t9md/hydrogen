...
function getExecuteRequestContent(code) {
  return {
    code,
    silent: false,
    store_history: true,
...
export default class ZMQKernel extends Kernel {
  ...
  //--------------------------
  sendMessageToShellSocket({msgType, msgId, content, callback}) {
    msgId = msgId || `${msgType}_${v4()}`
    if (callback) {
      this.executionCallbacks[msgId] = callback
    }
  ...
  execute(code, onResults) {
    this.sendMessageToShellSocket({
      ...
      content: getExecuteRequestContent(code),
      callback: onResults,
    })
  }
  ...
  inspect(code, cursorPosition, onResults) {
    this.sendMessageToShellSocket({
  ...
  handleShellMessage(message) {
    ...
    const callback = this.executionCallbacks[message.parent_header.msg_id]
    if (!callback) return
    callback(translateShellMessage(message))
  ...
  sendInputReply(input) {
    const msgId = `input_reply_${v4()}`
    const content = {value: input}
    const message = buildMessage("input_reply", msgId, content)
  ...
  handleIOMessage(message) {
    ...
    const callback = this.executionCallbacks[msg_id]
    if (!callback) return
    const parsedIOMessage = this._parseIOMessage(message)
    if (parsedIOMessage) {
      callback(parsedIOMessage)
    }
  }
}
