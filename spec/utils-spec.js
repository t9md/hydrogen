...
describe("utils", () => {
  describe("grammarToLanguage", () => {
    ...
    it("should return respect languageMappings", () => {
      ...
      expect(grammarToLanguage({name: "Grammar Language"})).toEqual("kernel language")
      expect(grammarToLanguage({name: "Kernel Language"})).toEqual("kernel language")
  ...
  it("getEmbeddedScope", () => {
    const editor = {
      scopeDescriptorForBufferPosition: () => {
        return {
          getScopesArray: () => ["text.md", "fenced.code.md", "source.embedded.python"],
        }
      },
    }
